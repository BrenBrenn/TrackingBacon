...
This endpoint renders a human-friendly dashboard with direct links, a **Run Sniffer Now** button, and a report panel.
...
### 4) Run once manually and get a report (button)

Open `http://127.0.0.1:8000/` in browser, then:
1. Click **Run Sniffer Now**
2. Wait for the report panel to update
3. You will see inserted/updated counts + sizzle event summary

You can also do this by API:
...
### 5) Reminder (The Sizzle) usage

On homepage:
1. (Optional) paste webhook URL in the input box
2. Click **Send Reminder**

API version:

```bash
curl -X POST "http://127.0.0.1:8000/alerts/test?webhook_url=https://your-webhook-url"
```

If no webhook is provided, the API returns a preview instead of sending.

### 6) Inspect detected products
